...
private:
    ...
    bool dfs(TreeNode* A, TreeNode* B){
        if(B == nullptr) return true; // 到叶子节点
...
用一个数组used标记字符是否使用过。当窗口最右边pos的值已经出现过，那么左边界要往右移动，同时更新窗口内最大不同子字符串数。
...
public:
    int lengthOfLongestSubstring(string s) {
        ...
        int left = 0, pos = 0, res = 0;
        while(pos < s.size()){
            // 当窗口最右边的元素在窗口内存在了,就应该向右移动left，直到排除掉重复的元素
            // 把之前的访问过的设置为没访问过
            while(used[s[pos]]){
                used[s[left]] = false;
                left++;
...
public:
    int lengthOfLongestSubstring(string s) {
        ...
        for(int j = 0; j < s.size(); j++){
            if(map.count(s[j]) != 0){
                 i = max(i, map[s[j]]); //如果存在这个字符，更新i为字符上一次出现的位置.上一次位置比i小则不更新，不然 j - i会更大
            }
            map[s[j]] = j; // 更新当前字符所在位置，下一次若再碰到这个字符，就知道上一次出现的位置
            res = max(res, j - i);
        }
        return res;
    }
