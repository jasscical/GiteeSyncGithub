...
3. 在集合n中**⾄多**要从该起始位置 : n - (k - path.size()) + 1，开始遍历（这个有些拗口，理解是至多，也就是说若是这个位置后开始开始，是不可能凑到k个元素的）
...
>
> used[i - 1] == false ，就说明：前⼀个树枝，使⽤了**candidates[i - 1]**，也就是说同⼀树层使⽤过**candidates[i - 1]**。
>
...
递归终止条件：：切割线切到了字符串最后⾯，说明找到了⼀种切割⽅法，此时就是本层递归的终⽌终⽌条件。
