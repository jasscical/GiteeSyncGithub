...
4. 遍历顺序：⾸先从递推公式中可以看出，`s[i]==s[j] && j - i > 1`是根据`dp[i + 1][j - 1]`是否为true，在对`dp[i][j]`进⾏赋值true的。`dp[i + 1][j - 1] `在 `dp[i][j]`的左下⻆，如图：![image-20210703215245607](回文系列1.assets/image-20210703215245607.png)
...
跟[131. 分割回文串](https://leetcode-cn.com/problems/palindrome-partitioning/)对比，131逻辑的逻辑是将整体进行分割，即把整个字符串切分，使得每一个都是回文子串，取的是整体；
